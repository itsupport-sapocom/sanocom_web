...
import { createBotProtectionState, getBotProtectionPayload } from '@/lib/contactBotProtection';
...
export default function ContactUsModal() {
    ...
    const [botProtection, setBotProtection] = useState(() => createBotProtectionState());
    ...
    async function formSubmitHandler(e) {
        ...
        try {
            ...
            const jsonResponse = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...form,
                    ...getBotProtectionPayload(botProtection),
                })
            })
            const response = await jsonResponse.json();
            if (!jsonResponse.ok) {
                throw new Error(response.message || 'Unable to submit the form right now.');
            }
            ...
            setBotProtection(createBotProtectionState());
        } catch (error) {
                        ...
                        <div className='md:m-4'>
                            ...
                            <form className='md:max-w-[450px]' onSubmit={formSubmitHandler}>
                                ...
                                <div className='hidden' aria-hidden="true">
                                    <label htmlFor="modalWebsite" className='mb-4 md:mb-6 block'>
                                        <span className='block opacity-60 text-lg'>Website</span>
                                        <input type="text" name="website" id="modalWebsite" tabIndex={-1} autoComplete="off" className='border-b border-b-black/60 w-full py-2 outline-none bg-transparent' value={botProtection.website} onChange={(e) => setBotProtection({ ...botProtection, website: e.target.value })} />
                                    </label>
                                </div>

                                <div className='mb-4 md:mb-6'>
                                    <span className='block opacity-60 text-lg mb-3'>Verification *</span>
                                    <ReCAPTCHA
                                        sitekey={process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY || ''}
                                        onChange={(token) => setBotProtection({ ...botProtection, recaptchaToken: token || '' })}
                                        onExpired={() => setBotProtection({ ...botProtection, recaptchaToken: '' })}
                                    />
                                </div>
