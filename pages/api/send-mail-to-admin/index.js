...
import { validateBotProtection } from "@/lib/contactBotProtection";
...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      const botProtectionValidation = await validateBotProtection(req.body);

      if (!botProtectionValidation.isValid) {
        return res.status(400).json({
          message: botProtectionValidation.message,
        });
      }
